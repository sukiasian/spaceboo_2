...
RouteProtector.confirmedUserProtector = this.utilFunctions.catchAsync(async (req, res, next) => {
    const user = await this.userModel.scope(user_model_1.UserScopes.WITH_CONFIRMED).findOne({
        where: {
            id: req.user.id,
        },
    });
